...
class FilmesController extends AbstractActionController
{
    ...
    public function indexAction()
    {
        ...
        if ($deleteId) {
            ...
            if ($filme) {
                $em->remove($filme);
                $em->flush();
                // Invalidar cache e sinalizar atualização para front-end
                try {
                    $cache = new FileCache();
                    $cache->clear();
                } catch (\Throwable $e) {
                    // silencioso: cache não é crítico
                }
                $this->touchFilmesUpdated();
            }
        ...
        if ($request->isPost()) {
            ...
            if ($action === 'add') {
                ...
                $em->persist($filme);
                $em->flush();
                // Invalidar cache e notificar alteração
                try {
                    $cache = new FileCache();
                    $cache->clear();
                } catch (\Throwable $e) {
    ...
    /**
     * Endpoint simples que retorna o timestamp da última alteração nos filmes.
     * Usado pelo front-end (home) para detectar mudanças e recarregar a lista.
     */
    public function lastUpdatedAction()
    {
        $file = __DIR__ . '/../../../../data/filmes_last_update.txt';
        $ts = 0;
        if (file_exists($file)) {
        ...
        return new JsonModel(['lastUpdated' => $ts]);
    }

    /**
     * Escreve o timestamp atual em arquivo compartilhado para sinalizar
     * que houve alterações na lista de filmes.
     */
    private function touchFilmesUpdated(): void
    {
        $file = __DIR__ . '/../../../../data/filmes_last_update.txt';
        try {
